...
const FooterCTA = () => {
  ...
  return (
    <section id="contact" className="relative w-full min-h-screen bg-transparent flex flex-col items-center justify-center overflow-hidden z-10 pt-24 pb-0 px-4">
      
      {/* Background ambient glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-2xl h-[600px] bg-brandCyan/10 blur-[120px] rounded-full pointer-events-none" />
      ...
      <div className="relative w-full max-w-4xl h-[700px] md:h-[750px] flex flex-col items-center justify-start mt-16 md:mt-24">
        ...
        <AnimatePresence>
          {!isSubmitted && (
            <motion.div
              ...
              className="relative w-[340px] md:w-[480px] bg-white/[0.03] backdrop-blur-2xl border border-white/10 rounded-3xl overflow-hidden shadow-[0_20px_60px_rgba(34,211,238,0.1)] z-20 cursor-grab active:cursor-grabbing flex flex-col"
            >
              ...
              {isScanning && (
                ...
                >
                  ...
                  <motion.div 
                    ...
                    className="w-full h-1 bg-brandCyan shadow-[0_0_30px_#22d3ee]"
                  />
                  <div className="absolute inset-0 bg-brandCyan/10 mix-blend-overlay" />
                </motion.div>
              )}
              ...
              <div className="w-full bg-black/60 p-5 md:p-6 border-b border-white/10 flex justify-between items-center relative">
                <div>
                  <h3 className="text-[10px] md:text-xs font-bold text-slate-400 tracking-widest uppercase">PROJECT TICKET</h3>
                  <h4 className="text-base md:text-lg font-bold text-white tracking-wider mt-1">CODE CANVAS</h4>
                </div>
                <QrCode className="w-8 h-8 md:w-10 md:h-10 text-brandCyan" />
              </div>
              ...
              <div className="p-6 md:p-8 flex flex-col gap-6 relative z-10 pointer-events-none">
                ...
                <div className="flex flex-col gap-1 pointer-events-auto">
                  <label className="text-[10px] font-bold text-slate-500 tracking-widest uppercase">YOUR NAME</label>
                  <input 
                    type="text" 
                    value={formData.name}
                    onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                    className="bg-transparent border-b border-white/20 text-slate-300 font-mono text-sm focus:outline-none focus:border-brandCyan transition-colors py-1 placeholder-slate-500"
                    placeholder="ENTER NAME"
                  />
                </div>

                <div className="flex flex-col gap-1 pointer-events-auto">
                  <label className="text-[10px] font-bold text-slate-500 tracking-widest uppercase">EMAIL ADDRESS</label>
                  <input 
                    type="email" 
                    value={formData.email}
                    onChange={(e) => setFormData({ ...formData, email: e.target.value })}
                    className="bg-transparent border-b border-white/20 text-slate-300 font-mono text-sm focus:outline-none focus:border-brandIndigo transition-colors py-1 placeholder-slate-500"
                    placeholder="ENTER EMAIL"
                  />
                </div>

                <div className="flex flex-col gap-1 pointer-events-auto">
                  <label className="text-[10px] font-bold text-slate-500 tracking-widest uppercase">PROJECT TYPE</label>
                  <input 
                    type="text" 
                ...
                {/* New Message Field */}
                <div className="flex flex-col gap-1 pointer-events-auto">
                  <label className="text-[10px] font-bold text-slate-500 tracking-widest uppercase">PROJECT MESSAGE</label>
                  <textarea 
                    rows="2"
                    value={formData.message}
                    onChange={(e) => setFormData({ ...formData, message: e.target.value })}
                    className="bg-transparent border-b border-white/20 text-slate-300 font-mono text-sm focus:outline-none focus:border-brandIndigo transition-colors py-1 placeholder-slate-500 resize-none"
              ...
              <div className="w-full bg-black/60 p-4 md:p-6 border-t border-white/10 flex flex-col items-center justify-center gap-2 relative">
                <div className="flex items-center gap-3">
                  ...
                  <p className="text-[10px] md:text-[12px] font-semibold text-slate-400 tracking-[0.4em] uppercase">DRAG TO SEND</p>
                </div>
              </div>
        ...
        <div className="absolute bottom-0 left-0 w-full h-32 md:h-40 flex flex-col items-center justify-start pointer-events-none z-30">
          ...
          >
            {/* Glowing inner edge */}
            <div className="w-[90%] h-[1px] bg-gradient-to-r from-transparent via-brandCyan to-transparent opacity-80" />
            <div className="absolute inset-0 bg-brandCyan/10 animate-pulse mix-blend-overlay" />
          </div>
          
          {/* Solid base block to hide the ticket as it slides behind (into) it */}
          <div className="w-full h-full bg-[#050505] border-t border-white/5 flex flex-col items-center pt-8 relative shadow-[0_-10px_40px_rgba(34,211,238,0.05)]">
            {/* Subtle glow casting up from the slot */}
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-64 h-1 bg-brandCyan/40 blur-md" />
            
            <div className="flex items-center gap-3">
              <ScanLine className="w-5 h-5 md:w-6 md:h-6 text-brandCyan animate-pulse" />
              <span className="text-[10px] md:text-xs font-bold text-brandCyan tracking-[0.4em] uppercase">
        ...
        {isSubmitted && (
          ...
          >
            <div className="w-20 h-20 md:w-24 md:h-24 rounded-full bg-brandCyan/10 border border-brandCyan/30 flex items-center justify-center shadow-[0_0_50px_rgba(34,211,238,0.2)] mb-8">
              <CheckCircle2 className="w-10 h-10 md:w-12 md:h-12 text-brandCyan" />
            </div>
