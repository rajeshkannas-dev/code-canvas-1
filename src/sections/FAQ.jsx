      ...
      <div className="w-full max-w-3xl pointer-events-auto">
        {faqs.map((faq, idx) => (
          <div key={idx} className="mb-4">
            <button
              className={`w-full text-left p-6 glass-card border transition-colors duration-300 flex justify-between items-center ${activeIndex === idx ? 'border-brandIndigo bg-white/10' : 'border-white/10 hover:border-white/30'}`}
              onClick={() => setActiveIndex(activeIndex === idx ? null : idx)}
            >
              <span className="text-sm font-semibold text-slate-300">{faq.question}</span>
              <motion.div
                animate={{ rotate: activeIndex === idx ? 180 : 0 }}
                transition={{ duration: 0.3 }}
              >
                <ChevronDown className="text-brandIndigo" />
              </motion.div>
            </button>
            <AnimatePresence>
              {activeIndex === idx && (
                ...
                >
                  <div className="p-6 text-sm text-slate-400 border-l border-r border-b border-brandIndigo/50 rounded-b-xl bg-darkBg/50 backdrop-blur-md">
                    {faq.answer}
                  </div>
                </motion.div>
              )}
